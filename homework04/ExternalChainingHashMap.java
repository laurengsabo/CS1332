...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        curr = table[index];
        ExternalChainingMapEntry<K, V> putEntry = new ExternalChainingMapEntry<>(key, value);
        putEntry.setNext(curr);
    ...
    public V remove(K key) {
        ...
        int index = Math.abs(key.hashCode() % table.length);
        ExternalChainingMapEntry<K, V> curr = table[index];
        ExternalChainingMapEntry<K, V> prior = null;
    ...
    public void resizeBackingTable(int length) {
        ...
        ExternalChainingMapEntry<K, V>[] newBackTable = new ExternalChainingMapEntry[length];

        for (ExternalChainingMapEntry<K, V> entry : table) {
            if (entry != null) {
                newBackTable[Math.abs(entry.getValue().hashCode() % table.length)] = entry;
            }
        }
        table = newBackTable;
    }
